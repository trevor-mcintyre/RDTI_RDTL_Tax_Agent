...
export default function Reports() {
  ...
  const handleExport = () => {
    ...
    const csvData = ["ID,Title,Status,Amount,Date", ...filtered.map(c => `${c.id},${c.title},${c.status},${c.amount},${c.date}`)].join("\n");
    const blob = new Blob([`${filterMeta}\n${csvData}`], { type: 'text/csv' });
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-6">
        ...
        <div>
          ...
          <select className="w-full p-2 border rounded" value={user} onChange={e => setUser(e.target.value)}>
            {['All', 'Alice', 'Bob'].map(s => <option key={s}>{s}</option>)}
          </select>
        </div>
        <div>
          <label className="block font-medium mb-1">Filter by Reviewer</label>
          <select className="w-full p-2 border rounded" value={reviewer} onChange={e => setReviewer(e.target.value)}>
        ...
        <div>
          ...
          <input type="date" value={start} onChange={e => setStart(e.target.value)} className="p-2 mr-2 border rounded" />
      ...
      <table className="w-full text-left border mb-6">
        ...
        <tbody>
          {filtered.length === 0 ? (
            <tr>
              <td colSpan={5}><EmptyState message='No claims match your filters 👀' /></td>
            </tr>
          ) : (
            filtered.map(claim => (
              <tr key={claim.id}>
